...
def mainMateLovers(opcUsuario, cantPrg):
  cleanAll()
  ...
  for i in range (0, cantPrg):
    ...
    elif(opcUsuario == 2): # Anti LaPlace
       # L^-1{k/s} = k
      if nRndPrg == 0:
    ...
    for k in range(4):
      if(opcUsuario == 1): # Laplace
        ...
        posiRes[k] = posiNum[k] + " / " + posiDen[int(random.uniform(0,len(posiDen)))]
...
while(opcUsuario != 4):
  ...
  # OPCION DE TUTORIAL
    ...
    while(opcUsuario2 != 3):
      ...
      print("2) Anti Laplace")
      print("3) Regresar")
      opcUsuario2 = int(input("Opcion: "))

      if(opcUsuario2 != 3):
        mainMateLovers(opcUsuario2, 1)
